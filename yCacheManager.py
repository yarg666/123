...
class yCacheManagerClass(QWidget): 
	...
	def __init__(self):
		...
		self.cacheBrowser.setText("cacheLoader:")
		self.quickPublishPartTwo = QCheckBox("cacheImportants",self)	
		...
		#add wdget part I:
		mainLayout.addWidget(self.cacheManagerLabel,0,0,1,1)
		#mainLayout.addWidget(self.filterKeyCurrent,0,1,1,1)
		mainLayout.addWidget(self.quickPublish,0,3,1,1)
		mainLayout.addWidget(self.update,1,1,1,1)
		mainLayout.addWidget(self.upVersionB,1,0,1,1)
		...
		#mainLayout.addWidget(self.upVersionB,5,0,1,2)
...
#PART II
	...
	def menuEffet(self):
		...
		#remplir list effets
		for i in xInfo:
			myEffet = i.attrib["effet"].split("_")[-2]
			self.effetName.addItem(myEffet)
		try:
			self.cacheBrowserF()
		except:
			pass	
	...
	def cacheBrowserF(self):
		...
		xInfo  = self.tree.xpath("/projetTree/projet[@name='%s']/shot[@shot='%s'][@user='%s']" %(projetSelect,shotSelect,userSelect))
		for i in xInfo:
			if effetSelect == i.attrib["effet"].split("_")[-2]:
				path = "/".join(i.attrib["path"].split("/")[:-1])
				print path
...
#PART I
	...
	def fileCacheLister(self):
		...
		try:
			while inc < len(self.cacheNameListe):
				...
				self.inUseVersion.activated.connect(self.updateVersionSolo)
				#Last version
				try:
				...
				if self.nodePath[inc].parm("quickPublish").eval() == 1:	
					self.openFolder.setText("explorer")
	...
	def changeVersion(self,row):
		...
		#get newVersion
		version=self.tableCurrent.cellWidget(ligne,2).currentText()
		#find node
		match = nodesearch.NodeType("CG::renderGeo*")
		net = hou.node("/obj/")

		for node in match.nodes(net,recursive =True):
			if str(node) == cacheNode:
				if node.parm("lecture").eval() == 1:
					try:
	...
	def updateVersionSolo(self):
		row=self.sender().property("row")
		self.changeVersion(row)

	def updateFoncBetaUp(self):
		rowCount = self.tableCurrent.model().rowCount()
		for row in range(rowCount):
			combo = self.tableCurrent.cellWidget(row,2)
			lastIndex= combo.count() -1
...
#BUTTON CONNECT
	def button(self):
		...
		self.upVersionB.clicked.connect(self.updateFoncBetaUp)
